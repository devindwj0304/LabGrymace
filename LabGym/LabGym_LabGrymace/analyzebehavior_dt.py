...
class AnalyzeAnimalDetector():
	...
	def prepare_analysis(self,
		...
		# Check if frame is valid before accessing shape
		if frame is None:
			raise ValueError("Could not read video frame. Video file may be corrupted or empty.")
		...
		if specific_behaviors is not None:
			self.specific_behaviors = specific_behaviors
		else:
			self.specific_behaviors = {}
			...
			if self.animal_present[animal_name]>0:
				...
				# Handle case when no existing centers (all animals are new)
					...
					for index_in_new in range(len(animal_centers)):
						...
						for i in range(self.animal_number[animal_name]):
							if self.animal_existingcenters[animal_name][i] == (-10000,-10000):
								self.register_counts[animal_name][i] = frame_count_analyze
								self.to_deregister[animal_name][i] = 0
								contour = animal_contours[index_in_new]
								center = animal_centers[index_in_new]
								self.animal_existingcenters[animal_name][i] = center
								if frame_count_analyze < len(self.animal_contours[animal_name][i]):
									self.animal_contours[animal_name][i][frame_count_analyze] = contour
								if frame_count_analyze < len(self.animal_centers[animal_name][i]):
									self.animal_centers[animal_name][i][frame_count_analyze] = center
								break
	...
	def categorize_behaviors(self, path_to_categorizer, uncertain=0, min_length=None):
		...
		if os.path.exists(_params_path):
			_mp=pd.read_csv(_params_path)
			if 'behavior_kind' in _mp.columns and int(_mp['behavior_kind'][0])==4:
				raise RuntimeError('This Categorizer uses a behavior mode that this build of LabGym does not support. This model cannot be used.')

		# Resolve .keras/.h5 model file inside the categorizer directory
		_model_file=path_to_categorizer
		if os.path.isdir(path_to_categorizer):
			for _fn in ['best_model.keras','final_model.keras']:
				_cand=os.path.join(path_to_categorizer,_fn)
				if os.path.exists(_cand):
					_model_file=_cand
					break
			else:
				for _r,_d,_fs in os.walk(path_to_categorizer):
					for _f in _fs:
						if _f.endswith('.keras') or _f.endswith('.h5'):
							_model_file=os.path.join(_r,_f)
							break
					break
		print(f'Loading categorizer model: {_model_file}')
		categorizer=load_model(_model_file)


		# =====================================================================
		# Standard frame-by-frame inference (modes 0, 1, 2, 3)
		# =====================================================================
		...
		for animal_name in self.animal_kinds:
			...
			# Free the per-animal source data as soon as it has been concatenated
			if self.animation_analyzer and animal_name in self.animations:
				del self.animations[animal_name]
			if animal_name in self.pattern_images:
				del self.pattern_images[animal_name]
			gc.collect()

			# Use GPU if available (removed CPU constraint for performance)
			if self.animation_analyzer:
			...
			idx = 0
			for n in IDs:
				register_count = self.register_counts[animal_name][n]
				if register_count is None:
					register_count = 0
				i = self.length + register_count
				idx += i
						...
						if check <= self.length / 2:
							...
							if len(behavior_names) == 2:
								if prediction[0] > 0.5:
									...
									predicted_behavior = behavior_names[0]
							else:
							...
							if behavior_allowed:
								...
								if len(behavior_names) == 2:
									...
									if confidence_diff > uncertain:
										...
										# Low confidence - mark as NA only if uncertain > 0
										if uncertain > 0:
											self.event_probability[animal_name][n][i] = ['NA', -1]
										else:
											# If uncertain == 0, always assign the most likely behavior
											if prediction[0] >= 0.5:
												self.event_probability[animal_name][n][i] = [behavior_names[1], prediction[0]]
											else:
												self.event_probability[animal_name][n][i] = [behavior_names[0], 1 - prediction[0]]
								else:
									# Optimize: sort once instead of twice
									sorted_pred = np.sort(prediction)
									confidence_diff = sorted_pred[-1] - sorted_pred[-2]
									if confidence_diff > uncertain:
			...
			del predictions
			del inputs
			# Note: Avoid gc.collect() here as it's very slow
			# TensorFlow will manage GPU memory automatically with growth enabled
			...
			for animal_name in self.animal_kinds:
				for n in IDs:
					...
					while i < len(self.event_probability[animal_name][n]):
						if self.event_probability[animal_name][n][i][0] == \
								self.event_probability[animal_name][n][i - 1][0]:
							continued_length += 1
						else:
							if continued_length < min_length:
								self.event_probability[animal_name][n][i - continued_length:i] = [['NA',
																								   -1]] * continued_length
							continued_length = 1
						i += 1
	...
	def annotate_video(self,animal_to_include,ID_colors,behavior_to_include,show_legend=True):
		...
		# Improved video capture with error handling
		capture=cv2.VideoCapture(self.path_to_video)
							...
							if frame_count_analyze<len(self.animal_contours[animal_name][i]):
								...
								if self.animal_contours[animal_name][i][frame_count_analyze] is not None:
									...
									if self.categorize_behavior:
										...
										# Safety check for event_probability access
										event_exists = (animal_name in self.event_probability and
														i in self.event_probability[animal_name] and
														frame_count_analyze < len(self.event_probability[animal_name][i]))
										if event_exists and self.event_probability[animal_name][i][frame_count_analyze][0]=='NA':
											if self.behavior_mode==1:
		...
		if writer is not None:
			...
			print("Warning: No frames were written to annotated video")
		...
		if len(parameter_to_analyze)>0:
			...
			for animal_name in self.animal_kinds:
				...
				for i in self.animal_contours[animal_name]:
					...
					register_count = self.register_counts[animal_name][i]
					if register_count is None:
						register_count = 0
					n=self.length+register_count
									...
									if contour is not None:
										...
										for ct in self.animal_contours[animal_name][i][n-self.length+1:n+1]:
											if ct is not None:
												...
												#diff[diff!=0]=255
												prev_mask_sum = np.sum(prev_mask)/255
												if prev_mask_sum > 0:
													area_diff=(np.sum(diff)/255)/prev_mask_sum
					...
					if frame_count_analyze>=self.length and frame_count_analyze%skip_redundant==0:
						...
						for n in self.animal_centers[animal_name]:
							...
							if h>0:
								...
								if getattr(self,'generate_perframe_roi',False):
									base=os.path.splitext(pattern_image_name)[0]
									cap_avi=cv2.VideoCapture(path_animation)
									fi=0
									while True:
										ret,avi_frame=cap_avi.read()
										if not ret:
											break
										cv2.imwrite(os.path.join(self.results_path,str(animal_name)+'_'+str(n),base+'_f'+str(fi)+'.jpg'),avi_frame)
										fi+=1
									cap_avi.release()
