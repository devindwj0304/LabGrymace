	...
	def generate_data(self,event):
		...
		if self.path_to_videos is None or self.result_path is None:
			...
			if self.behavior_mode>=3:
				...
				dialog=wx.MessageDialog(self,'Generate per-frame ROI images?\nEach animation will also be saved as individual\nframes showing the animal\'s real appearance at each timestep.','Per-frame ROI?',wx.YES_NO|wx.ICON_QUESTION)
				self.generate_perframe_roi=dialog.ShowModal()==wx.ID_YES
				dialog.Destroy()
				...
				if do_nothing is False:
					...
					for i in self.path_to_videos:
						...
						if self.use_detector is False:
							...
							AA.generate_perframe_roi=self.generate_perframe_roi
							...
							AAD.generate_perframe_roi=self.generate_perframe_roi
							if self.behavior_mode==0:
