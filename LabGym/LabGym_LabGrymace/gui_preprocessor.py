...
def find_ffmpeg():
	...
	# but the standalone ffmpeg in the environment usually can.
...
def is_readable_video(path):
	...
	if capture.isOpened():
		...
		readable=bool(retval) and frame is not None
	capture.release()
	return readable
...
def ensure_readable_videos(parent,paths):
	...
	# transcoded once to an H.264 sibling file ('<name>_h264.mp4') via standalone ffmpeg, and the converted
	# path is used instead. Returns the (possibly updated) list of readable video paths.
	good_paths=[]
	failed=[]
	ffmpeg_bin=None
	...
	for path in paths:
		...
		# This video cannot be decoded by OpenCV; convert it to H.264.
		converted=os.path.splitext(path)[0]+'_h264.mp4'

		# Reuse a previously converted, readable copy if it already exists.
		if os.path.exists(converted) and is_readable_video(converted):
			good_paths.append(converted)
			continue

		if ffmpeg_bin is None:
			ffmpeg_bin=find_ffmpeg()
		if ffmpeg_bin is None:
			failed.append((path,'no ffmpeg binary found to convert the video'))
			continue

		command=[ffmpeg_bin,'-y','-i',path,'-c:v','libx264','-pix_fmt','yuv420p','-crf','18','-preset','fast','-an',converted]
		progress=wx.ProgressDialog('Converting video','Converting incompatible video to H.264 (runs once per file):\n'+os.path.basename(path),maximum=100,parent=parent,style=wx.PD_AUTO_HIDE|wx.PD_ELAPSED_TIME)
		# ffmpeg writes its (often very verbose) log to stderr; send it to a temp file rather than a pipe.
		# An unread PIPE fills its OS buffer and deadlocks ffmpeg, so we never read stderr inline here.
		errlog=tempfile.NamedTemporaryFile(mode='w+',suffix='.log',delete=False)
		try:
			proc=subprocess.Popen(command,stdout=subprocess.DEVNULL,stderr=errlog)
			...
			try:
				errlog.close()
				os.remove(errlog.name)
			except Exception:
				pass

		if returncode==0 and is_readable_video(converted):
			good_paths.append(converted)
		else:
			if os.path.exists(converted):
				try:
					...
					pass
			failed.append((path,(err.strip().splitlines()[-1] if err.strip() else 'conversion failed')))

	if failed:
		message='The following video(s) could not be read or converted and were skipped:\n\n'
		for path,reason in failed:
			message+=os.path.basename(path)+'  ('+reason+')\n'
		wx.MessageBox(message,'Unreadable video(s)',wx.OK|wx.ICON_WARNING)
...
class WindowLv2_ProcessVideos(wx.Frame):
	...
	def select_videos(self,event):
		...
		if dialog.ShowModal()==wx.ID_OK:
			...
			selected_paths.sort()
			self.path_to_videos=ensure_readable_videos(self,selected_paths)
			if not self.path_to_videos:
				self.path_to_videos=None
				self.text_inputvideos.SetLabel('None. (selected video(s) could not be read).')
				dialog.Destroy()
				return
