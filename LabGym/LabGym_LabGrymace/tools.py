...
# environment already sets one (HPC modules, conda, system installs all do); only fall
# back to probing the usual locations. Nothing is forced when CUDA is absent -- TensorFlow
# then runs on CPU, which is the correct behaviour on a machine without a GPU.
_cuda_home = os.environ.get('CUDA_HOME') or os.environ.get('CUDA_PATH')
if not _cuda_home:
	for _candidate in ('/usr/local/cuda', '/opt/cuda'):
		if os.path.isdir(_candidate):
			_cuda_home = _candidate
			break
if _cuda_home:
...
def crop_frame(frame,contours):
	...
	lfbt=np.array([c.min(0) for c in valid_contours]).min(0)[0]
	x_lf=lfbt[0]
	y_bt=lfbt[1]
	rttp=np.array([c.max(0) for c in valid_contours]).max(0)[0]
	x_rt=rttp[0]
	y_tp=rttp[1]
...
def generate_patternimage_perframe(frame,outlines):
	...
	'''
	Returns a list of individual images, one per frame in the time window.
	Each image shows only that single frame's contour drawn in its temporal color
	(blue for first frame, transitioning to red for the last), cropped to the
	shared bounding box of all contours so all images are the same size.
	'''
	...
	(y_bt,y_tp,x_lf,x_rt)=crop_frame(frame,valid_outlines)
	...
	for n,outline in enumerate(outlines):
		...
		if outline is not None:
			...
			cv2.drawContours(panel,[outline],0,color,p_size)

		images.append(panel[y_bt:y_tp,x_lf:x_rt])
...
def generate_patternimage_interact_perframe(frame,outlines,other_outlines):
	...
	'''
	Per-frame version of generate_patternimage_interact.
	Returns a list of N images (one per frame in the time window).
	Each image shows:
	  - the main organ contour in its temporal color (blue first, red last)
	  - all other organ contours in grey (150,150,150), same as the combined pattern image
	All images share the same bounding box so they are the same size.
	'''

	if frame is None:
		return []

	valid_main=[o for o in outlines if o is not None]
	if not valid_main:
		return []

	total_outlines=functools.reduce(operator.iconcat,[ol for ol in other_outlines if ol is not None],[])
	total_outlines+=list(valid_main)
	...
	for n,outline in enumerate(outlines):
		...
		if len(other_outline)>0 and other_outline[0] is not None:
			cv2.drawContours(panel,other_outline,-1,(150,150,150),p_size)
...
def preprocess_video(
	...
	bottom=0,
	fps_new=None,
	enhance_brightness=None,  # Add compatibility parameter
	brightness=1.0,  # Add brightness parameter
	):
