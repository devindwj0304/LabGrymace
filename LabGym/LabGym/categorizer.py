...
def _load_model(path):
	...
	and pins execution to CPU to avoid CUDA conflicts with PyTorch."""
	import os
		...
		class _Wrapper:
			def predict(self_inner, inputs, batch_size=32, verbose=0):
				...
				for start in range(0, n, batch_size):
					if is_list:
						batch = {k: inputs[i][start:start+batch_size] for i, k in enumerate(in_keys)}
					else:
						batch = {in_keys[0]: inputs[start:start+batch_size]}
					with tf.device('/CPU:0'):
						out = infer(**batch)
					results.append(out[out_keys[0]].numpy())
	...
	class _CPUWrapper:
		...
		def predict(self_inner, inputs, batch_size=32, verbose=0):
			...
			for start in range(0, n, batch_size):
				...
				with tf.device('/CPU:0'):
					out = self_inner._m(batch, training=False)
...
class Categorizers():
		...
		if len(list(self.classnames))<2:
			...
			if out_folder is None:
				...
				if len(self.classnames)==2:
					model.compile(optimizer=SGD(learning_rate=1e-4,momentum=0.9),loss='binary_crossentropy',metrics=['accuracy'],jit_compile=False,run_eagerly=True)
				else:
					model.compile(optimizer=SGD(learning_rate=1e-4,momentum=0.9),loss='categorical_crossentropy',metrics=['accuracy'],jit_compile=False,run_eagerly=True)

				cp=ModelCheckpoint(os.path.join(model_path,'best_model.keras'),monitor='val_loss',verbose=1,save_best_only=True,save_weights_only=False,mode='min',save_freq='epoch')
 ...
 # also emit the diagnostic confusion matrix on the (augmented) validation split
					try:
						if len(self.classnames)==2:
							...
							cm_pred=np.array(predictions).astype(int)
						else:
							cm_true=testY.argmax(axis=1)
							cm_pred=predictions.argmax(axis=1)
						self._plot_diagnostic_confusion_matrix(cm_true,cm_pred,list(self.classnames),report,out_path)
					except Exception as cm_error:
						print('Confusion matrix generation failed: '+str(cm_error))
 ...
 # Fix UnicodeEncodeError by specifying UTF-8 encoding
	...
	def test_categorizer(self,groundtruth_path,model_path,result_path=None):
		...
		elif behavior_mode==3:
			print('The behavior mode of the Categorizer: Static images (non-interactive).')
		else:
			print('The behavior mode of the Categorizer: Unsupported (behavior mode '+str(behavior_mode)+'). This Categorizer cannot be used.')
		...
		if len(incorrect_behaviors)==0 and len(incorrect_classes)==0:
			...
			if len(classnames)==2:
				pred_labels=[round(i[0]) for i in predictions]
				print(classification_report(labels,pred_labels,target_names=classnames))
				report=classification_report(labels,pred_labels,target_names=classnames,output_dict=True)
			else:
			...
			if result_path is not None:
				...
				self._plot_diagnostic_confusion_matrix(labels,pred_labels,classnames,report,result_path)
	...
	def _plot_diagnostic_confusion_matrix(self,true_labels,pred_labels,classnames,report,result_path):
		...
		# figure matches publication-style diagnostic confusion matrices.
		...
		for i in range(n):
			for j in range(n):
				...
				if i==j:
					f1=report.get(str(classnames[i]),{}).get('f1-score',None)
		...
		plt.close(fig)
		print('Diagnostic confusion matrix saved to: '+str(save_path))
