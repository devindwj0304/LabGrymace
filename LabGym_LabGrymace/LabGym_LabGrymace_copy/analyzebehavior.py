...
class _SavedModelWrapper:
	...
	def __init__(self, path):
		...
		if not had_add_slot:
			...
			def _dummy_add_slot(self, *args, **kwargs):
				...
				if var is not None:
					try:
						return tf.Variable(tf.zeros_like(var), trainable=False)
					except Exception:
						pass
				return None
			autotrackable.AutoTrackable.add_slot = _dummy_add_slot
		try:
			with tf.device('/CPU:0'):
				self._saved = tf.saved_model.load(path)
				self._infer = self._saved.signatures['serving_default']
				self._in_keys = list(self._infer.structured_input_signature[1].keys())
				self._out_keys = list(self._infer.structured_outputs.keys())
		finally:
	...
	def predict(self, inputs, batch_size=32, verbose=0):
		...
		n = inputs[0].shape[0] if is_list else inputs.shape[0]
		results = []
		for start in range(0, n, batch_size):
			if is_list:
				batch = {k: inputs[i][start:start+batch_size] for i, k in enumerate(self._in_keys)}
			else:
				batch = {self._in_keys[0]: inputs[start:start+batch_size]}
			with tf.device('/CPU:0'):
...
class _KerasModelWrapper:
	...
	def predict(self, inputs, batch_size=32, verbose=0):
		...
		results = []
		for start in range(0, n, batch_size):
			batch = [inp[start:start+batch_size] for inp in inputs] if is_list else inputs[start:start+batch_size]
			with tf.device('/CPU:0'):
				out = self._model(batch, training=False)
...
def _load_model(path):
	...
	if os.path.isdir(path):
		return _SavedModelWrapper(path)
	return _KerasModelWrapper(load_model(path))
...
class AnalyzeAnimal():
	...
	def categorize_behaviors(self,path_to_categorizer,uncertain=0,min_length=None):
		...
		params_path=os.path.join(path_to_categorizer,'model_parameters.txt')
		if os.path.exists(params_path):
			_params=pd.read_csv(params_path)
			if 'behavior_kind' in _params.columns and int(_params['behavior_kind'][0])==4:
		...
		categorizer=_load_model(path_to_categorizer)
	...
	def annotate_video(self,ID_colors,behavior_to_include,show_legend=True,interact_all=False):
		...
		# Create a copy of background for trajectory drawing !New
		trajectory_background = self.background.copy()
						...
						if frame_count_analyze<len(self.animal_contours[i]):
							...
							if self.animal_contours[i][frame_count_analyze] is not None:
								...
								if self.animal_centers[i][max(frame_count_analyze-1,0)] is not None:
									...
									cv2.line(trajectory_background,(cx,cy),(cxp,cyp),(abs(int(color_diff*(total_animal_number-current_animal_number)-255)),int(color_diff*current_animal_number/2),int(color_diff*(total_animal_number-current_animal_number)/2)),int(text_tk))
									cv2.line(frame,(cx,cy),(cxp,cyp),(abs(int(color_diff*(total_animal_number-current_animal_number)-255)),int(color_diff*current_animal_number/2),int(color_diff*(total_animal_number-current_animal_number)/2)),int(text_tk))
								else:
									cv2.circle(trajectory_background,(cx,cy),int(text_tk),(abs(int(color_diff*(total_animal_number-current_animal_number)-255)),int(color_diff*current_animal_number/2),int(color_diff*(total_animal_number-current_animal_number)/2)),-1)
									cv2.circle(frame,(cx,cy),int(text_tk),(abs(int(color_diff*(total_animal_number-current_animal_number)-255)),int(color_diff*current_animal_number/2),int(color_diff*(total_animal_number-current_animal_number)/2)),-1)
		...
		cv2.imwrite(os.path.join(self.results_path,'Trajectory.jpg'),trajectory_background)
							...
							if h>0:
								...
								if getattr(self,'generate_perframe_roi',False):
									...
									while True:
										...
										if not ret:
											break
										cv2.imwrite(os.path.join(self.results_path,'examples',str(n),base+'_f'+str(fi)+'.jpg'),avi_frame)
										fi+=1
									cap_avi.release()
						...
						if h>0:
							...
							if getattr(self,'generate_perframe_roi',False):
								...
								while True:
									...
									if not ret:
										break
									cv2.imwrite(os.path.join(self.results_path,'examples','0',base+'_f'+str(fi)+'.jpg'),avi_frame)
									fi+=1
								cap_avi.release()
